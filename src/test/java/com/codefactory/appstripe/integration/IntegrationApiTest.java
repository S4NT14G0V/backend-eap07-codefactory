...
class IntegrationApiTest {
    ...
    void cp_s2_003_transactionWithWrongMerchantCredentials() {
        Map<String, Object> tx = new HashMap<>();
        tx.put("merchantId", "mch_intruso_123");
        tx.put("amount", 10000);
        ...
        given()
            ...
            .when().post("/api/v1/transactions")
            .then().statusCode(401)
                .body("errorCode", equalTo("CREDENTIAL_MISMATCH"));
    }

        // ========================================================================
    // SPRINT 2 — HU010: Revocación de credenciales
    // ========================================================================

    @Test
    @Order(13)
    @DisplayName("CP-S2-010: Revocación exitosa de credencial -> HTTP 200 + active=false")
    void cp_s2_010_revokeCredential() {
        String revokedPublicId = publicId;

        given()
                .cookie("XSRF-TOKEN", csrfCookie)
                .header(csrfHeaderName, csrfToken)
                .header("Authorization", "Bearer " + adminToken)
            .when().patch("/api/v1/admin/credentials/{publicId}/revoke", revokedPublicId)
            .then().statusCode(200)
                .body("publicId", equalTo(revokedPublicId))
                .body("active", equalTo(false));
    }

        @Test
    @Order(14)
    @DisplayName("CP-S2-011: Credencial revocada rechaza pago -> HTTP 401 INVALID_CREDENTIALS")
    void cp_s2_011_transactionWithRevokedCredential() {
        Map<String, Object> tx = new HashMap<>();
        tx.put("merchantId", merchantId);
        tx.put("amount", 10000);
